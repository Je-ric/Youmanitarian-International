        ...
        <form method="POST" action="{{ route('programs.feedback.submit', $program->id) }}"
            ...
            <div class="rating mb-6 flex justify-center">
                @for ($i = 1; $i <= 5; $i++) <input type="radio" name="rating" value="{{ $i }}"
                    class="mask mask-star bg-yellow-400" {{ $userFeedback ? 'disabled' : '' }} {{ ($userFeedback &&
                    $userFeedback->rating == $i) ? 'checked' : '' }} required />
                    @endfor
            </div>
            ...
            <textarea name="feedback" rows="4"
                ...
                placeholder="Share your thoughts about this program..." {{ $userFeedback ? 'readonly' : ''
                }}>{{ $userFeedback ? $userFeedback->feedback : '' }}</textarea>
...
<dialog id="feedbackModal_{{ $program->id }}" class="modal p-0">
    <div
        ...
        <form method="POST" action="{{ route('programs.feedback.submit', $program->id) }}"
                ...
                @if($userFeedback)
                    <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                        <div class="flex items-center gap-2 text-green-700">
                            ...
                            <span class="font-medium text-sm">Thank you for your feedback! We truly appreciate your
                                contribution.</span>
                        </div>
                    </div>
                @endif
            ...
            <footer
                class="border-t border-slate-200 px-6 py-4 bg-slate-50 flex justify-between items-center flex-shrink-0">
                <button type="button" onclick="document.getElementById('feedbackModal_{{ $program->id }}').close();"

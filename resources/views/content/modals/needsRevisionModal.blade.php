...
@if(isset($content))
<x-modal.dialog id="needs-revision-modal-{{ $content->id }}" maxWidth="xl:max-w-xl lg:max-w-lg md:max-w-md sm:max-w-sm max-w-xs" width="w-full" maxHeight="max-h-[90vh]">
    <x-modal.header>
        ...
        <div class="space-y-4">
            <div class="flex items-center gap-3 p-3 bg-amber-50 border border-amber-200 rounded">
                ...
                <p class="text-sm text-amber-700">
                    ...
                    <span class="font-semibold">Needs Revision</span>. It will not be published until resubmitted and approved.
                </p>
            </div>

            <div class="bg-gray-50 border border-gray-200 rounded p-3">
                <p class="text-xs uppercase tracking-wide text-gray-500 mb-1">Title</p>
                <p class="text-sm font-semibold text-gray-800 line-clamp-2">{{ $content->title }}</p>
            </div>
    ...
    <x-modal.footer>
        <div class="flex gap-2 w-full justify-end flex-col sm:flex-row">
            ...
            <button
                ...
                type="submit"
                class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2 rounded-md font-semibold bg-amber-500 hover:bg-amber-600 text-white transition">

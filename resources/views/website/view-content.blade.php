...
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-gray-100">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12">
        ...
        <div class="mb-6 sm:mb-8">
            <a href="{{ url('/') }}" 
               class="inline-flex items-center gap-2 px-4 py-2 bg-[#1a2235] text-white rounded-lg hover:bg-[#2a3441] transition-all duration-200 shadow-sm hover:shadow-md text-sm font-medium">
        ...
        <div class="flex flex-col xl:flex-row gap-6 lg:gap-8">
            ...
            <article class="xl:w-2/3">
                <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                    ...
                    <!-- Article Header -->
                    <header class="p-6 sm:p-8 lg:p-10">
                        <div class="flex flex-wrap items-center gap-3 mb-4">
                            <span class="inline-flex items-center px-3 py-1 bg-[#ffb51b] text-white text-sm font-semibold rounded-full">
                                {{ ucfirst($content->content_type) }}
                            </span>
                            @if($content->is_featured)
                            <span class="inline-flex items-center px-3 py-1 bg-gradient-to-r from-red-500 to-red-600 text-white text-sm font-semibold rounded-full">
                                <i class='bx bx-star mr-1'></i> Featured
                            </span>
                            @endif
                        </div>

                        <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#1a2235] leading-tight mb-4">
                            {{ $content->title }}
                        </h1>
                    ...
                    <div class="px-6 sm:px-8 lg:px-10 pb-6 sm:pb-8">
                        <div class="border-t border-gray-200 pt-6 sm:pt-8">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
                                ...
                                <div class="group relative overflow-hidden rounded-xl cursor-pointer transform hover:scale-105 transition-all duration-300 shadow-md hover:shadow-lg"
                    ...
                    <div class="px-6 sm:px-8 lg:px-10 py-6 border-t border-gray-200 bg-gray-50">
                        <div class="flex flex-col sm:flex-row justify-between gap-4">
                            ...
                            <a href="{{ route('website.view-content', $prevContent->slug) }}"
                               class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 hover:border-gray-400 transition-all duration-200 text-sm font-medium">
                            ...
                            <a href="{{ route('website.view-content', $nextContent->slug) }}"
                               class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 hover:border-gray-400 transition-all duration-200 text-sm font-medium sm:ml-auto">
            ...
            <aside class="xl:w-1/3">
                <div class="bg-white rounded-2xl shadow-lg border border-gray-200 sticky top-6 max-h-[calc(100vh-3rem)] overflow-hidden">
                    ...
                    <div class="overflow-y-auto max-h-[calc(100vh-12rem)]">
                        <div class="p-6 space-y-4">
                            ...
                            <a href="{{ route('website.view-content', $item->slug) }}" 
                               class="block group">
                                <article class="p-4 rounded-xl border border-gray-200 hover:border-[#ffb51b] hover:shadow-md transition-all duration-200">
                                    <div class="flex gap-4">
                                        @if ($item->image_content)
                                        <div class="w-16 h-16 sm:w-20 sm:h-20 flex-shrink-0 overflow-hidden rounded-lg">
                                            <img src="{{ \App\Http\Controllers\WebsiteController::getImageUrl($item->image_content) }}" 
                                                 alt="Thumbnail" 
                                                 class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-200">
                                        </div>
                                        @else
                                        <div class="w-16 h-16 sm:w-20 sm:h-20 flex-shrink-0 bg-gray-100 rounded-lg flex items-center justify-center">
                                            <i class='bx bx-image text-xl text-gray-400'></i>
                                        </div>
                                        @endif

                                        <div class="flex-1 min-w-0">
                                            <h3 class="font-semibold text-[#1a2235] text-sm sm:text-base line-clamp-2 group-hover:text-[#ffb51b] transition-colors duration-200">
                                                {{ $item->title }}
                                            </h3>
                                            ...
                                            @if($item->is_featured)
                                            <span class="inline-flex items-center px-2 py-1 bg-red-100 text-red-700 text-xs rounded-full mt-2">
                                                <i class='bx bx-star mr-1'></i> Featured
                                            </span>
                                            @endif
                                        </div>
...
<div id="imageModal" class="fixed inset-0 bg-black bg-opacity-90 items-center justify-center hidden z-50 p-4">
    <button onclick="closeImageModal()" 
            class="absolute top-4 right-4 text-white text-2xl sm:text-3xl hover:text-[#ffb51b] transition-colors z-10 w-10 h-10 flex items-center justify-center rounded-full bg-black bg-opacity-50">
        <i class='bx bx-x'></i>
    </button>
    
    <button onclick="prevImage()" 
            class="absolute left-4 top-1/2 transform -translate-y-1/2 text-white text-2xl sm:text-3xl hover:text-[#ffb51b] transition-colors z-10 w-10 h-10 flex items-center justify-center rounded-full bg-black bg-opacity-50">
        <i class='bx bx-chevron-left'></i>
    </button>
    
    <img id="modalImage" class="max-w-full max-h-full rounded-xl shadow-2xl" src="/placeholder.svg" alt="Preview">
    
    <button onclick="nextImage()" 
            class="absolute right-4 top-1/2 transform -translate-y-1/2 text-white text-2xl sm:text-3xl hover:text-[#ffb51b] transition-colors z-10 w-10 h-10 flex items-center justify-center rounded-full bg-black bg-opacity-50">
        <i class='bx bx-chevron-right'></i>
    </button>
</div>
...
<script>
    ...
    function editComment(commentId) {
        ...
        inputField.classList.add("w-full", "p-3", "border-2", "border-gray-200", "rounded-lg", 
        ...
        cancelButton.classList.add("px-4", "py-2", "bg-gray-200", "text-gray-700", "rounded-lg", 
